    ...
    while (TRUE)
    {
        ...
        while (!session_close)
        {
            ...
            case CWMP_ST_START:
                ...
                cwmp_log_debug("session stutas: New START\n");
...
int cwmp_agent_analyse_session(cwmp_session_t * session)
{
    ...
    doctmppool = pool_create(POOL_DEFAULT_SIZE*2);

    /* The netcwmp code do not expect <XML_VERSION> thus they add <cwmp> before xmlbuf. But 
    the genieACS sends the <XML_VERSION> thus the <cwmp> add after <XML_VERSION>*/

    //TODO: Modify the code as it support both with/without "XML_VERSION" xml. Currently support only with "XML_VERSION" xml(which is true for GenieACS).
    xmlbuf_helper = pool_palloc(doctmppool, msglength);
    xmlbuf = pool_palloc(doctmppool, msglength+32+strlen(XML_VERSION));

	cwmp_chunk_copy(xmlbuf_helper, session->readers, msglength);
	strncpy(xmlbuf,xmlbuf_helper,strlen(XML_VERSION));

	len = sprintf(xmlbuf + strlen(XML_VERSION),"<cwmp>");
	strncpy( xmlbuf + strlen(XML_VERSION) + len, xmlbuf_helper + strlen(XML_VERSION), msglength - strlen(XML_VERSION) );
	strcpy(xmlbuf + len + msglength , "</cwmp>");

	pool_pfree(doctmppool, xmlbuf_helper);
	xmlbuf += strlen(XML_VERSION);
#if 0
    ...
    cwmp_log_debug("agent analyzze xml: \n%s", xmlbuf);
			...
			case TASK_REBOOT_TAG:
				{
					...
					printf("Rebooting the system...\n");
     					sync();  // Syncing the filesystem before rebooting
					reboot(RB_AUTOBOOT);  // Trigger a reboot
